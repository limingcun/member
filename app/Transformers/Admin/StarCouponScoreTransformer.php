...
class StarCouponScoreTransformer extends TransformerAbstract
{

    public function transform(MemberScore $member_score)
    {
        $flag = false;
        // 没有使用赠饮券  order library policy ！= fee
        // 单笔消费大于11元    order_item price >= 11
        $order = Order::where('id', $member_score->source_id)
            ->select('coupon_library_id')->first();
        if (strlen($order->coupon_library_id) > 1) {
            $library_id = explode(',', $order->coupon_library_id);
            $library = CouponLibrary::whereIn('id', $library_id)
                ->where('policy', 'App\Policies\CouponLibrary\FeeCouponPolicy')
                ->exists();
            if (!$library) {
                $items = $order->item;
